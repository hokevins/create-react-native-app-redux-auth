...
![iPhone X Screenshot DEMO 1](assets/demo1.png)![iPhone X Screenshot DEMO 2](assets/demo2.png)![iPhone X Screenshot DEMO 3](assets/demo3.png)
...
Simply running `create-react-native-app-redux-auth` in a directory creates a sub-folder `generated` containing your new application files.
...
```bash
$ create-react-native-app-redux-auth path/to/any/directory/you/prefer
```
...
* In your `package.json`, change the "name" to your app's name.
* In your `IP.js`, change the IP to your backend's full IP address, whether that's on `localhost:PORT` or someplace deployed, like Heroku.
...
* Login will make a *PUT* request to `${IP}/auth/login` with the payload { email: email, password: password }.
* Signup will make a *POST* request to `${IP}/auth/signup` with the payload { email: email, password: password }.
* Delete will make a *DELETE* request to `${IP}/auth/logout` with no payload.
